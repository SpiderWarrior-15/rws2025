...
import { User, Edit, Save, X, Star, Trophy, MessageCircle, Calendar, MapPin, Mail, Phone, Crown, Award, Target, Users, Zap, Camera, Lock, Eye, EyeOff, UserPlus } from 'lucide-react';
...
export const Profile: React.FC = () => {
  ...
  useEffect(() => {
    // Check if viewing another user's profile via URL params
    const urlParams = new URLSearchParams(window.location.search);
    const userId = urlParams.get('user');
    
    if (userId && userId !== user?.id) {
      // Load other user's profile
      dataService.getUserById(userId).then(otherUser => {
        if (otherUser) {
  ...
  const getRoleDisplay = () => {
    if (!profileUser) return 'Warrior';
    return profileUser.role === 'admin' ? 'Spider Warrior (Admin)' : 'Warrior';
  };

  const getRoleBadgeColor = () => {
    if (!profileUser) return 'bg-blue-500/20 text-blue-400';
      ...
      <div className="min-h-screen pt-20 pb-12 flex items-center justify-center">
        <GlassCard className="p-8 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            This warrior's profile could not be found
          </p>
        </GlassCard>
      </div>
    );
            ...
            <GlassCard className="p-6 text-center">
                ...
                <div className="relative w-32 h-32 mx-auto mb-4">
                  ...
                  >
                    <span className="text-4xl font-bold text-white">
                      {(profileUser.avatar?.name || profileUser.username).charAt(0).toUpperCase()}
                    </span>
                  </div>
              ...
              <div className="mb-6">
                {isEditing && isOwnProfile ? (
                  <textarea
                    ...
                    className="w-full px-3 py-2 bg-white/10 dark:bg-gray-800/50 backdrop-blur-md border border-white/20 dark:border-gray-700/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-gray-800 dark:text-white resize-none"
                    rows={3}
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="text-center p-3 bg-white/5 dark:bg-gray-800/20 rounded-xl">
                  ...
                  <div className="text-lg font-bold text-gray-800 dark:text-white">{profileUser.stats.totalPoints}</div>
                ...
                <div className="text-center p-3 bg-white/5 dark:bg-gray-800/20 rounded-xl">
                  ...
                  <div className="text-lg font-bold text-gray-800 dark:text-white">{profileUser.stats.achievementsUnlocked}</div>
                  <div className="text-xs text-gray-600 dark:text-gray-400">Achievements</div>
                </div>
                <div className="text-center p-3 bg-white/5 dark:bg-gray-800/20 rounded-xl">
                  <Target className="w-6 h-6 text-green-500 mx-auto mb-1" />
                  <div className="text-lg font-bold text-gray-800 dark:text-white">{profileUser.stats.puzzlesSolved}</div>
                ...
                <div className="text-center p-3 bg-white/5 dark:bg-gray-800/20 rounded-xl">
                  ...
                  <div className="text-lg font-bold text-gray-800 dark:text-white">{profileUser.stats.messagesCount}</div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="font-medium text-gray-800 dark:text-white">{profileUser.email}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="font-medium text-gray-800 dark:text-white">
                      {format(new Date(profileUser.lastActive), 'MMM dd, yyyy HH:mm')}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className={`font-medium ${profileUser.isOnline ? 'text-green-400' : 'text-gray-400'}`}>
                ...
                {profileUser.location && (
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="font-medium text-gray-800 dark:text-white">{profileUser.location}</div>
                    </div>
                  </div>
                )}
              </div>
            ...
            <GlassCard className="p-6">
              ...
              <div className="space-y-4 max-h-64 overflow-y-auto">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-blue-600">{profileUser.stats.messagesCount}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Forms Submitted</span>
                  <span className="font-bold text-green-600">{profileUser.stats.formsSubmitted}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Events Attended</span>
                  <span className="font-bold text-purple-600">{profileUser.stats.eventsAttended}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Songs Uploaded</span>
                  <span className="font-bold text-pink-600">{profileUser.stats.songsUploaded}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Puzzles Solved</span>
                  <span className="font-bold text-yellow-600">{profileUser.stats.puzzlesSolved}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Quizzes Completed</span>
                  <span className="font-bold text-teal-600">{profileUser.stats.quizzesCompleted}</span>
                </div>
                <div className="flex justify-between items-center border-t border-gray-700 pt-4">
                  <span className="text-gray-600 dark:text-gray-400 font-medium">Total Points</span>
                  <span className="font-bold text-2xl text-purple-600">{profileUser.stats.totalPoints}</span>
                </div>
              </div>
            </GlassCard>
          </div>
